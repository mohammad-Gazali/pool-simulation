...
- `src/physics/` — simulation engine (equations, collision detection, motion integration).
  ...
  - `simulation.ts` — `PhysicsLoop` component (useFrame + fixed-timestep 120 Hz accumulator) runs `stepBall()` per ball: determines rest/spin/rolling-without-slip/rolling-with-slip state, applies appropriate friction forces, couples linear/angular velocity. Follows PHYSICS.md Section 3: rest → spinning (ω_y only) → rolling-with-slip (μ_s) → rolling-without-slip (μ_r). Vector-based stopping check in rolling state. Uses `GRAVITY_SCENE` for friction forces. Accesses zustand state via `store.getState()` to avoid hook subscription re-renders in the tick loop
