...
- `src/physics/` — simulation engine (equations, collision detection, motion integration).
  ...
  - `ball-collision.ts` — `handleBallCollisions()` detects overlapping ball pairs (distance < 2·R) and applies Section 4 impulse response. Uses relative velocity in the struck ball's frame; swaps the normal component multiplied by `(1 + E_BALL_BALL)/2` (≈0.975). Angular velocities unchanged (friction neglected per Section 4 assumptions). Separates overlapping balls by moving each outward by `overlap/2` along the center-to-center normal. Accumulates fixes in a `Map` so a ball involved in multiple collisions sees all prior updates during the same pass.
  - `cushion-collision.ts` — `handleCushionCollisions()` detects ball penetration past felt boundaries (±1.28 × ±0.595) and applies Section 5 impulse response: normal reflection with restitution `E_BALL_CUSHION`, Coulomb friction on the tangential component using `MU_SLIDING`, and resulting angular velocity change from the friction torque applied at the contact point `r = R·n̂`. Each cushion (4 edges) checked independently; position clamped to boundary after collision
  - `simulation.ts` — `PhysicsLoop` component (useFrame + fixed-timestep 120 Hz accumulator) runs `stepBall()` per ball. State machine follows PHYSICS.md Section 3: rest → spinning (ω_y only) → rolling-with-slip (μ_s) → rolling-without-slip (μ_r). Contains a `STICTION_BAND` threshold — forward Euler in the slipping state creates a limit cycle when relSpeed < (7/2)·μ_s·g·dt because a single timestep overshoots zero; STICTION_BAND force-enters the rolling state when relSpeed enters this irresolvable band. Vector-based stopping check returns null (no store update) only when all of speed, |wx|, |wy|, |wz| are below thresholds. Uses `GRAVITY` directly (no conversion). After each rolling/slipping step, passes the result through `handleCushionCollisions()` to reflect off felt boundaries. **After all balls are stepped, runs a ball-ball collision pass** via `handleBallCollisions()` and re-checks cushion boundaries for any ball moved by ball-ball separation. Accesses zustand state via `store.getState()` to avoid hook subscription re-renders
